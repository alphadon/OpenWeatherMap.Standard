...
namespace OpenWeatherMap.Standard.Tests
{
    ...
    public class ZipCodeTests
    {
        ...
        public void TestCloudy()
        {
            ...
            A.CallTo(() => fake.GetCurrentWeatherAsync("http://api.openweathermap.org/data/2.5/weather?zip=32927,USA&appid=UnitTest&units=Standard")).Returns(Task.FromResult(expect));
            var weather = new OpenWeatherMap.Standard.WeatherService(fake);

            string actual = weather.GetCurrentWeatherByZipAsync("UnitTest", "32927", "USA", WeatherUnits.Kelvin).Result.WeatherConditions[0].Description;
