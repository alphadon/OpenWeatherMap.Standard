...
namespace OpenWeatherMap.Standard.Tests
{
    ...
    public class CityNameTests
    {
        ...
        public void TestCloudy()
        {
            ...
            A.CallTo(() => fake.GetCurrentWeatherAsync("http://api.openweathermap.org/data/2.5/weather?q=Cocoa,FL,USA&appid=UnitTest&units=Standard")).Returns(Task.FromResult(expect));
            var weather = new OpenWeatherMap.Standard.WeatherService(fake);
            string actual = weather.GetCurrentWeatherByCityNameAsync("UnitTest", "Cocoa,FL", "USA", WeatherUnits.Kelvin).Result.WeatherConditions[0].Description;
